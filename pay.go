...
// Pay makes payment request to the Zeno API using the global configurations passed on the
// zeno.ConfigAPI() function
...
// PayWithConfig makes payment request to the Zeno API using the configurations passed on the function
func PayWithConfig(paymentData PaymentData, config Options, timeoutFn func(orderID string)) (err error) {

	// construct data in buffer of url.Values
	data := newDataWithConfig(paymentData, config)
